...
graph TB
    ...
    subgraph CloudTier ["☁️ Cloud & Database Tier (Supabase & Firebase)"]
        C1[("Supabase Real-Time DB<br/>guardian_users, emergency_alerts<br/>otp_records, contacts")]
        C2[("Firebase Firestore & Auth<br/>Push FCM Tokens, User Store")]
    end
...
```mermaid
flowchart TD
    Start(["Launch GuardianAI App"]) --> CheckAuth{"Is Session Active?<br/>(Firebase / OTP / Demo)"}
...
- **Interactive Leaflet.js Radar**: Displays live victim SOS beacons with expanding red radar waves and green shield pins for Guardian responders.
- **Live Emergency Feed**: Instant action triggers for *"Dispatch Nearest Guardian"* and *"Confirm Safe Resolution"*.
- **User Directory & Multi-Role Manager**: Filter, search, and switch roles between `SuperAdmin`, `Guardian`, and `User`.
